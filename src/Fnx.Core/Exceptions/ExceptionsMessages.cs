namespace Fnx.Core.Exceptions
{
    internal static class ExceptionsMessages
    {
        internal const string WrongType = "Wrong type";

        internal const string OptionIsNone = "Option is None";

        internal const string TryIsBottom = "Try state is wrong";
        internal const string TryIsNotSuccess = "Try is not Success";
        internal const string TryIsNotFailure = "Try is not Failure";

        internal const string ResultIsError = "Result is Error";
        internal const string ResultIsOk = "Result is Ok";

        internal const string ValidatedIsInvalid = "Validated is Invalid";
        internal const string ValidatedIsValid = "Validated is Valid";
    }
}